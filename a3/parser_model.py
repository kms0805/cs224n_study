...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        if cuda:
            w = w.cuda()
        flatten_w = torch.flatten(w) # (batch_size * n_features)
        x = torch.index_select(self.embeddings, 0, flatten_w) # (batch_size * n_feautres, embed_size)
    ...
    def forward(self, w):
        ...
        h = self.dropout(F.relu(x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)) #(batch_size, hidden_size)
