...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        min_batch_size = min(len(unfinished_parses), batch_size)
        minibatch = unfinished_parses[:min_batch_size]
        transitions = model.predict(minibatch)
        for pp, transition in zip(minibatch, transitions):
            pp.parse_step(transition)
        unfinished_parses[:min_batch_size] = [pp for pp in minibatch if pp.buffer or len(pp.stack) > 1 ]
